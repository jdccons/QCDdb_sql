...
GO
/****** Object:  StoredProcedure [dbo].[usp_GetUserPermissions]    Script Date: 01/11/2016 21:18:52 ******/
SET ANSI_NULLS ON
GO
...
-----------------------------------------------------------
-- OBJECT NAME :dbo.usp_GetUserPermissions
-- AUTHOR: Harsha Majety
-- DATE: 12/15/2010
-- INPUT PARAMETERS:         
...
AS 
    ...
    SELECT
        @DatabaseUserID = su.[uid], @ServerUserName = sl.[loginname]
    FROM
        dbo.[sysusers] su
        INNER JOIN [master].dbo.[syslogins] sl
            ON su.[sid] = sl.[sid]
    WHERE
        su.[name] = @DatabaseUserName
    ...
    ELSE 
        BEGIN
            SET @msgStatement = '--Security creation script for user '
                ...
                + ']' + CHAR(13) + 'EXEC [sp_grantdbaccess]' + CHAR(13)
                + CHAR(9) + '@loginame = ''' + @ServerUserName + ''','
                + CHAR(13) + CHAR(9) + '@name_in_db = ''' + @DatabaseUserName
                + '''' + ';' + CHAR(13) + 'GO' + CHAR(13)
                + '--Add User To Roles'
            ...
            WHILE @@FETCH_STATUS = 0 
                BEGIN
                    SET @msgStatement = 'EXEC [sp_addrolemember]' + CHAR(13)
                        + CHAR(9) + '@rolename = ''' + @RoleName + ''','
                        + CHAR(13) + CHAR(9) + '@membername = '''
                        + @DatabaseUserName + '''' + ';';
       ...
       --Database level perms;
            ...
            DECLARE _databaselevelperms CURSOR LOCAL FORWARD_ONLY READ_ONLY
            FOR
                SELECT
                    sdp.state_desc, sdp.permission_name
                FROM
            ...
            FETCH NEXT FROM _databaselevelperms INTO @StateDesc,
                @PermissionName;
      
            WHILE @@FETCH_STATUS = 0 
                BEGIN 
      
                    PRINT @StateDesc + CHAR(13) + CHAR(9) + @PermissionName
                        + CHAR(13) + CHAR(9) + 'TO ' + @userName + ';';
